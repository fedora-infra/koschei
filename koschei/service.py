...
class Service(object):
    ...
    def run_service(self):
        ...
        retry_attempts = 0
        print("{name} started".format(name=name))
        while True:
            try:
                retry_attempts = 0
                self.main()
    ...
    def find_service(cls, name):
        ...
        for subcls in cls.__subclassess__:
...
class KojiService(Service):
    ...
    def __init__(self, log=None, db_session=None, koji_session=None):
        super(KojiService, self).__init__(log, db_session)
        self.koji_session = util.Proxy(koji_session or self.create_koji_session())

    @classmethod
    def create_koji_session(cls):
        return util.create_koji_session(cls.koji_anonymous)
